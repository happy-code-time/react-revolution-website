...
class PagerStatic extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            itemsPerSite: (props.itemsPerSite && typeof 8 === typeof props.itemsPerSite) ? props.itemsPerSite : 10,
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (getDerivedStateFromPropsCheck(['defaultClass', 'id', 'data', 'searchOnKeys','itemsPerSite','keysToRender','displayLineNumber','liOnClickCallback','inputOnChangeCallback','displayTotal','displayPaginationPages','totalPrefix','paginationTextPrefix','paginationTextMiddle','prevPages','nextPages','alignPagesItems','alignPagination','resetLineNumber','nextButton','previousButton','displaySearch','searchPlaceholder','searchIcon','searchSensisitve','searchValue','searchOnKeyDown','searchTitle','paginationTitle','fallbackLoading','fallbackLoadingTime','fallbackNoData','fallbackNoDataSearch','fallbackMounting' ], props, state)) {
            return {
                addClass: (props.addClass && typeof '8' == typeof props.addClass) ? props.addClass : '',
                defaultClass: (props.defaultClass && typeof '8' == typeof props.defaultClass) ? props.defaultClass : 'rr-pager-static',
                id: (props.id && typeof '8' == typeof props.id) ? props.id : '',
     ...
     */
    getPagerJsx() {
        let { itemsPerSite, currentPage, filteredData, displayTotal, totalPrefix, displayPaginationPages, prevPages, nextPages, alignPagesItems, fallbackNoData, previousButton, nextButton, searchValue, fallbackNoDataSearch, dataLoaded, fallbackMounting } = this.state;
        ...
        if (maxPages) {
             ...
             * Prev pages
             */
            for (let x = mainPage - prevPages; x < mainPage; x++) {
            ...
            for (let x = 0; x <= maxPages; x++) {
                ...
                if (x > mainPage && x <= mainPage + nextPages) {
                    pagesNext.push(x);
                }
            }
        }
     ...
     */
    getList() {
        let { keysToRender, displayLineNumber, currentPage, itemsPerSite, filteredData, resetLineNumber } = this.state;
